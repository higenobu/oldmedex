...
(&$dbh, $p_oid, $p_pid, $time_from, $time_to, $options=NULL) {
  ...
  $sql = 'SELECT M.orderdate, M.bui1,  M.bui2, M.bui3,M.bui4, M."ObjectID" as oid , M."ID" as id
          FROM xctorder as M
          WHERE M."Superseded" is NULL AND
                M."患者" = ' . mx_db_sql_quote($p_oid) . " $tf $tt " . '
                ORDER BY M."ObjectID"';
  ...
  foreach ($rehabs as $rehab) {
    $uri = sprintf("/u/doctor/xctorder.php?pid=%s&detail=%s&oid=%s",
		   $p_oid,$rehab['oid'],$rehab['oid']);
	$text = sprintf("(%s ) %s",
		    $rehab['bui1'],
		    $rehab['bui2']);
  // $text ="ABC: from xct";
    $result[] = array('timestamp' => $rehab['orderdate'],
		      'text' => $text,
		      'callback_url' => $uri,
		      'thumb' => NULL,
		      'object_id' => $rehab['oid']
