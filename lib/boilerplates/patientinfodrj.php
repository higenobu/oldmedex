...
function mx_draw_patientinfo_bmd($ObjectID, $options=NULL)
{
  ...
  print "<tr><th>分類番号</th><td>";
  print htmlspecialchars($d['患者ID']);
//  print "</td><th>開始日</th><td>";
//  $x = $d['生年月日'];
//  if(mx_check_option('Culture', $options) == 'Japanese')
 //   $x = mx_wareki($x);
//  $x .=' (' . mx_calc_age($d['生年月日']) . ')';
//  print htmlspecialchars($x);
 print "</td><th>大分類</th><td>";
print htmlspecialchars($d['姓']);
/*
  print "</td><th rowspan=\"2\" style=\"vertical-align: middle\">大分類名</th>";
  print "<td rowspan=\"2\" style=\"vertical-align: middle; ";
  print "font-size: 150%; font-weight: bold; \">";
  print htmlspecialchars($d['名']);
  print "</td>\n";
*/
  ...
  print "<tr><th>中分類</th><td>";
print htmlspecialchars($d['名']);
//  print htmlspecialchars($d['フリガナ']);
  print "</td><th>小分類</th><td>";
print htmlspecialchars($d['フリガナ']);
