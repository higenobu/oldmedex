...
//$this->modality_Name='kenshin';
	...
	function left_pane() {
	  mx_formi_hidden($this->prefix . 'appt-to-modify', mx_form_escape_key($this->appt_to_modify));
		...
		print '<div>';
		if ($this->loo_pi->chosen()) {
			mx_titlespan($this->modality_type . 'を選択');
	...
	function show_calendar_pane() {
		...
		mx_titlespan("予約状況 (". $this->modality_Name . ")");
...
//$modality=500;
	...
	function done_processing() {
		...
		print "<a href=\"../../u/test/order2.php?$param2\">検査依頼</a>";
	...
	function show_appt($data, $prefix='') {
		...
		foreach (array('patient_ID' => '患者ID',
			       'patient_Name' => '患者名',
			       'modality_Name' =>
			       $data["$prefix" . "modality_Type"],
			       'apptdate' => '予約日',
			       'appttime' => '予約時刻') as $key => $label) {
			print "<tr>\n";
	...
	function right_pane() {
	  if ($this->cancelled_apptment){
		...
		if ($this->loo_modality->chosen() || $this->loo_appt->chosen()) {
			...
			print "</td></tr>\n";
			print "<tr><th>予約日</th><td>";
			mx_formi_date('apptdate', $this->data['apptdate']);
			print "</td></tr>\n";
			print "<tr><th>予約時刻</th><td>";
			$this->appttime();
			print "から";
			$this->apptdur();
			print "分間</td></tr>\n";
			print "<tr><th>予約目的</th><td>";
