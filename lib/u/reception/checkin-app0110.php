...
class reception_checkin_application extends single_table_application {
	...
	function left_pane() {
		...
		if ($this->mode == 'ask-patient-id') {
			...
			mx_titlespan('Select Rsved/checkin');
			$this->loo_ci->draw();
		} else {
			mx_titlespan('Select non-Rsved ');
			$this->loo_pi->draw();
			if ($this->loo_pi->chosen()) {
				print "<hr />\n";
				$this->loo_dr->draw();
	...
	function draw_checkout() {
		...
		if ($_mx_keep_done_patients_on_reception_list)
			$end_and_del = 'finished';
		else
			$end_and_del = 'finish and delete';
		mx_formi_submit('CheckoutPatient', $end_and_del);
		mx_formi_submit('CancelCheckin', 'delete checkin');
		mx_formi_submit('Cancel', 'close without action');
		$this->ctl_div_1();
	}

	function draw_checkin() {
		$this->msg_div_0();
		print ("change to checked in？");
		mx_formi_hidden('checkin_ID', $this->checkin_ID);
		$this->msg_div_1();

		$this->ctl_div_0();
		mx_formi_submit('CheckinPatient', 'checkin');
		mx_formi_submit('CancelAppt', 'delete checkin/rsv');
		mx_formi_submit('Cancel', 'close without action');
		$this->ctl_div_1();
	}
...
SQL;
	...
	function immediate_checkin($patient, $modality, $comment) {
		...
		$kamoku= '01';
		$kadoctor='11001';
		if ($modality=='60'){$kamoku='01';$kadoctor='11001';}	
		if ($modality=='61'){$kamoku='08';$kadoctor='11002';}
		if ($modality=='62'){$kamoku='07';$kadoctor='11003';}
		if ($modality=='63'){$kamoku='18';$kadoctor='11004';}
		
		$stmt = 'INSERT INTO "受付患者表"
("CreatedBy", "目的",
 "患者", "患者ID", "受付時刻", "予約先", "姓", "名", "フリガナ","受診科目コード","doctor_id")
SELECT ' . mx_db_sql_quote($me) . ' , ' . mx_db_sql_quote($comment) .
', "ObjectID", "患者ID", now(), ' .
mx_db_sql_quote($modality) . ', "姓", "名", "フリガナ",'."'$kamoku',"."'$kadoctor' ".
'FROM "患者台帳"
WHERE "Superseded" IS NULL AND "ObjectID" = ' . mx_db_sql_quote($patient);
	...
	function switch_to_waiting($patient) {
		$this->switch_state($patient, 'waiting');
	}

	function switch_to_done($patient) {
		$this->switch_state($patient, 'finished');
	}
