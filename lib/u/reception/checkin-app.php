...
//0110-2014
function mx_draw_patientinfo_custom2($ObjectID, $show,$title)
{
  ...
  foreach ($show as $a) {
    ...
    for ($i = 0; $i < $show_cnt; $i++) {
	    print "<th>" . htmlspecialchars($title[$i]) . "</th><td>";
...
class reception_checkin_application extends single_table_application {
	...
	var $_browse_only = 1;
	var $show_patient_cols = array(array("¥µº‘ID", "ª·Ãæ"),
				        );
	...
	function left_pane() {
		...
		if ($this->mode == 'ask-patient-id') {
			...
			mx_titlespan('select reserved/checkin');
			$this->loo_ci->draw();
		} else {
			mx_titlespan('select nonreserved');
			$this->loo_pi->draw();
			if ($this->loo_pi->chosen()) {
				print "<hr />\n";
				$this->loo_dr->draw();
		...
		if ($this->mode == 'ask-patient-id')
			...
			if (!$this->draw_immediate_at_top) {
				mx_formi_submit('ImmediateCheckIn',
						'checkin fot nonreserved');
			}
		}
		else
			mx_formi_submit('AppointmentCheckIn',
					'checkin for reserved/checkin');
	}
	...
	function choose_checkin() {
		...
		$this->patient_ID = trim($v['¥µº‘ID']);
		$this->patient_ObjectID = trim($v['¥µº‘']);
		$this->patient_Name = trim($v['¿´']) . trim($v['Ãæ']);
		$this->checkin_ID = $v['ObjectID'];
		$this->checkin_Time = $v['ºı…’ª˛πÔ'];
		$this->appt_Time = $v['ÕΩÃÛª˛πÔ'];
...
//0110-2014
	$tl[0]='PID';$tl[1]='Name'; 
		mx_draw_patientinfo_custom2($this->patient_ObjectID,
					   $this->show_patient_cols,$tl);
	...
	function draw_checkout() {
		...
		if ($_mx_keep_done_patients_on_reception_list)
			$end_and_del = 'completed';
		else
			$end_and_del = 'delete after complete';
		mx_formi_submit('CheckoutPatient', $end_and_del);
		mx_formi_submit('CancelCheckin', 'delete checkin');
		mx_formi_submit('Cancel', 'close/no action');
		$this->ctl_div_1();
	}

	function draw_checkin() {
		$this->msg_div_0();
		print ("this patient is checked in°©");
		mx_formi_hidden('checkin_ID', $this->checkin_ID);
		$this->msg_div_1();

		$this->ctl_div_0();
		mx_formi_submit('CheckinPatient', 'checkin');
		mx_formi_submit('CancelAppt', 'delete reserve/checkin');
		mx_formi_submit('Cancel', 'close/no action');
		$this->ctl_div_1();
	}
	...
	function draw_immediate() {
		...
		if (!$this->modality_Name) {
			...
			$this->ctl_div_0();
			mx_formi_submit('Cancel', 'no action/close');
			$this->ctl_div_1();
			return;
		}
		...
		$this->ctl_div_0();
		mx_formi_submit('CheckinImmediate', 'checkin');
		mx_formi_submit('Cancel', 'close/no action');
		$this->ctl_div_1();
	}

	function update_checkin_time($id, $t='now()') {
		$stmt = 'UPDATE "ºı…’¥µº‘…Ω" SET "ºı…’ª˛πÔ" = ' . $t .', checkin='.
' date(now()) '.' WHERE "ObjectID" = ' . mx_db_sql_quote($id);
//0115-2014
//print $stmt;
		$db = mx_db_connect();
		pg_query($db, $stmt);
	...
	function pid_from_checkin_id(&$db, $checkin_ID) {
		...
		$stmt = <<<SQL
		SELECT "¥µº‘" FROM "ºı…’¥µº‘…Ω" WHERE "ObjectID" = $checkin_ID
SQL;
		$result = mx_db_fetch_single($db, $stmt);
		if ($result && ($pid = $result["¥µº‘"]))
			return $pid;
		return NULL;
	}
...
SQL;
	...
	function record_checkout($id) {
		$stmt = 'UPDATE "ºı…’¥µº‘…Ω" SET "•π•∆°º•ø•π" = 1, ' .
			'"ø«Œ≈Ω™Œªª˛πÔ" = now() ' .
	...
	function delete_checkin_record($id) {
		...
		$stmt = <<<SQL
		SELECT rsched FROM "ºı…’¥µº‘…Ω" WHERE "ObjectID" = $id
SQL;
		$result = mx_db_fetch_single($db, $stmt);
...
SQL;
		...
		$stmt = ('DELETE FROM "ºı…’¥µº‘…Ω" WHERE "ObjectID" = ' .
			 $id);
		pg_query($db, $stmt);
	}
...
//0115-2014
		$stmt = 'INSERT INTO "ºı…’¥µº‘…Ω"
("CreatedBy", "Ã‹≈™",
 "¥µº‘", "¥µº‘ID", "ºı…’ª˛πÔ",checkin, "ÕΩÃÛ¿Ë", "¿´", "Ãæ", "•’•Í•¨• ")
SELECT ' . mx_db_sql_quote($me) . ' , ' . mx_db_sql_quote($comment) .
', "ObjectID", "¥µº‘ID", now(),date(now()), ' .
mx_db_sql_quote($modality) . ', "¿´", "Ãæ", "•’•Í•¨• "
FROM "¥µº‘¬Êƒ¢"
WHERE "Superseded" IS NULL AND "ObjectID" = ' . mx_db_sql_quote($patient);
	...
	function switch_to_waiting($patient) {
		$this->switch_state($patient, 'wait for exam');
	}

	function switch_to_done($patient) {
		$this->switch_state($patient, 'completion');
	}
