...
(
 'TABLE' => '患者台帳',
 'COLS' => array("患者ID", "姓", "名",country,tel, 
		 "性別", "利き手", "生年月日","birth",
		 "住所0", "住所1",
		 "住所2", "住所3", "住所4",
		 "加入電話",
...
array('Column' => "ptage",
					'Label' => 'Age',
				    'Compare' => 'EXTRACT(YEAR FROM AGE("生年月日"))',
				    'Draw' => 'text',
				    'Option' => array('size' => 3),
				    ),
...
('SELECT "ObjectID", "CreatedBy", ' .
 ...
 ', ("姓" || "名") AS "姓名"' .
',EXTRACT(YEAR FROM AGE("生年月日")) AS "ptage" '.
...
if ($data["携帯電話"] && !strpos("携帯電話"],"-")  ) {
$data["携帯電話"]=str_replace("-","",$data["携帯電話"]);
      $data["tel"]=sprintf("%03d-%03d-%04d",
...
if ($data["請求先電話番号"] && !strpos("請求先電話番号"],"-")  ) {
$data["請求先電話番号"]=str_replace("-","",$data["請求先電話番号"]);
      $data["tel"]=sprintf("%03d-%03d-%04d",
