...
(
 ...
 'LCOLS' => array_merge(array("職員ID", "姓名"),
			$_lib_u_manage_employee_ldcol_d),
 'DCOLS' => array_merge(array("職員ID"),
			$_lib_u_manage_employee_ldcol_d,
			array("職種名"),
			$_lib_u_manage_employee_ldcol_r,
			array("姓名", "フリガナ", "性別", "生年月日",
			      "住所0", "住所1", "住所2", "住所3", "住所4",
			      "加入電話", "携帯電話",
...
class employee_display extends simple_object_display {
  ...
  function annotate_row_data(&$row) {
    _lib_u_manage_employee_fix_abbrev_dept(&$row);
    switch ($row["性別"]) {
    case 'M': $row["性別"] = '男'; break;
    case 'F': $row["性別"] = '女'; break;
    case '男': break;
    case '女': break;
    default: $row["性別"] = '不明'; break;
    }
  }
