...
function getYoubi($date){
    $sday = strtotime($date);
    $res = date("w", $sday);
    $day = array("Æü", "·î", "²Ð", "¿å", "ÌÚ", "¶â", "ÅÚ");
    return $day[$res];
}
$busho_type = array(' ','1³¬', '2³¬', '¸¡ºº',  'ÉÔÌÀ');
...
'ALLOW_SORT' => 1,
 'DEFAULT_SORT' => 'seqno',
 'COLS' => array('employee_id','seqno','reportdate','patientnm','facttime', 'factloc','facttype','factcnt', 'factdone','factplan','factdo')
);
...
function _annotate_row_data(&$it, &$row) {
  ...
  $row['Éô½ð'] = $it->umap[$row['employee_id']]['ÃæÊ¬Îà1'];
