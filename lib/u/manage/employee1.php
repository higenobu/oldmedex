...
E."ObjectID", E."CreatedBy", "職員ID",
"姓", "名", "フリガナ", "性別", "生年月日",
"住所0", "住所1", "住所2", "住所3", "住所4",
"加入電話", "携帯電話",
("姓" || \' \' || "名") as "姓名",
E.userid, U.username as "ユーザ名",
E."職種", C."職種" as "職種名",
E."職位", R."職位" as "職位名",
E."部署",
D."コード" as "部署コード",
...
function _lib_u_manage_employee_fix_abbrev_dept(&$row)
{
  ...
  if (! array_key_exists("部署名", $row))
    $row["部署名"] = _lib_u_manage_department_abbrev
...
if (!$_mx_use_one_employee_dept) {
	$_lib_u_manage_employee_ldcol_d[] = '部署名';
}
if (!$_mx_use_one_employee_rank) {
	$_lib_u_manage_employee_ldcol_r[] = '職位名';
}
*/
...
(
 ...
 'COLS' => array(userid, "職員ID", 
       "職種", "職位", "部署", "姓", "名",  "性別", "生年月日", "加入電話"
	 
 ),

 'LCOLS' => array(
array('Column' =>  "職員ID",'Label'=>'ID'),
array('Column' =>  "姓",'Label'=>'LastName'),

array('Column' =>  "名",'Label'=>'FirstName'),
...
/*
 'ICOLS' => array(
array('Column'=>"userid",'Label'=>"userid"),
array('Column' =>  "職員ID",'Label'=>'ID'),
array('Column' =>  "姓名",'Label'=>'Name'),
array('Column' =>  "性別",'Label'=>'Gender'),
array('Column' =>  "生年月日",'Label'=>'DOB'),
array('Column' =>  "電話",'Label'=>'Tel'),
array('Column'=>"職種名",'Label'=>"Emptype"),
array('Column'=>"職位",'Label'=>"Position"),
array('Column'=>"部署",'Label'=>"Dept"),
array('Column' =>  "加入電話",'Label'=>'Tel'),
),
 ...
 'LCHOICE' => array('' => 'all employee',
		    ),

 'ALLOW_SORT' => array('職員ID' => array('ID' => '"ID"'),
...
(
 'Emptypename' => array('cf' => $_lib_u_manage_employee_cat_cfg,
		   'cls' => 'list_of_employee_cats',
		   'twm' => 'この職種に設定する',
		   'col' => '職種'),
 'Positionname' => array('cf' => $_lib_u_manage_employee_rank_cfg,
		   'cls' => 'list_of_employee_ranks',
		   'twm' => 'この職位に設定する',
		   'col' => '職位'),
 'Deptname' => array('cf' => $_lib_u_manage_department_cfg,
...
//0102-2014
$_mx_use_one_employee_dept=1;
  $_mx_use_one_employee_rank=1;
  $_mx_use_employee_class_choice=1;
    if ($_mx_use_one_employee_dept)
	    $this->cols["部署名"] = 0;
    if ($_mx_use_one_employee_rank)
	    $this->cols["職位名"] = 0;
...
SQL;
  ...
  function _validate() {
    ...
    if (trim($d['生年月日']) == '')
	    $this->data['生年月日'] = NULL;
    else if ($msg = mx_db_validate_date($d['生年月日'])) {
	    $this->err("生年月日: $msg");
	    $errs++;
	    $this->data['生年月日'] = NULL;
	    $this->note_badcol('生年月日');
    }
	...
	if (! is_null($result) && $result['count'] != 0) {
      ...
      // Passwords must be non-empty and match
      // ... also in u/change-password.php
      if ($msg = mx_db_validate_length($d['パスワード'],
				       $_mx_min_length_of_employee_pw, -1)) {
	$this->err("パスワード: $msg");
	$this->note_badcol('パスワード');
	$this->note_badcol('パスワード(もう一度)');
	$errs++;
      }
      elseif ($d['パスワード'] != $d['パスワード(もう一度)']) {
	$this->err('パスワード: ２つのパスワードが一致しません');
	$this->note_badcol('パスワード');
	$this->note_badcol('パスワード(もう一度)');
	$errs++;
      }
