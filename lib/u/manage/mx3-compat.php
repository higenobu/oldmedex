...
include_once $_SERVER['DOCUMENT_ROOT'].'/lib/u/manage/ms2.php';
...
class mx_mx3_compat extends single_table_application {
	...
	function mx_mx3_compat($config) {
		single_table_application::single_table_application();
		$this->loo = new master_select('mx3-', $config);
	}
...
function master_select_table($param, $qbe_enum=NULL, $header_fields=NULL) {
  ...
  $config = array
    (
     ...
     'COLS' => array_merge(array('selected'), $param['column']),
     'accept_name' => 'selected',
  ...
  $mx3_compat = new mx_mx3_compat($config);
  $mx3_compat->main();
}
