...
//04042014 modified shijii, doctor list
//for hayashi?
// 
...
$_mx_employee_labosystemcode= "LaboSystemCode";
  if ($_mx_employee_labosystemcode)
    ...
    $id_col = 'ObjectID';
// LaboSystemCode is recorded at test-order

  $db = mx_db_connect();
  $stmt = <<<SQL
    select E."LaboSystemCode" as id , E."姓" || E."名" as name
...
function _lib_u_test_get_doctors12() {
  global $_mx_employee_labosystemcode;
$_mx_employee_labosystemcode= "LaboSystemCode";
  if ($_mx_employee_labosystemcode)
    $id_col = $_mx_employee_labosystemcode;
...
// LaboSystemCode is recorded at test-order
  ...
  $db = mx_db_connect();
  $stmt = <<<SQL
    select E."ID" as id , E."姓" || E."名" as name
    from "職員台帳" E JOIN "職種一覧表" T ON E."職種" = T."ObjectID"
     JOIN modalities_to_medex_employee M on E."ID" =M."employee" 
    AND E."Superseded" IS NULL
    AND T."Superseded" IS NULL
...
// if config.mk is set to Labosystemcode, then record labosysttemcode, else record objectid
...
function _lib_u_test_get_doctors3() {
  global $_mx_employee_labosystemcode;
...
SQL;
*/
//check modality exists for emplaoyee 04042014
$stmt = <<<SQL
    select E."ID" as id , E."姓" || E."名" as name
    from "職員台帳" E JOIN "職種一覧表" T ON E."職種" = T."ObjectID"
     JOIN modalities_to_medex_employee M on E."ID" =M."employee" 
...
function _lib_u_test_get_doctors2() {
  global $_mx_employee_labosystemcode;
...
(
 ...
 'COLS' => array('OrderDate', 'SampleDate', 'scheduled', 'urgent', 'Type',
		 ...
		 'Orderer','clip_num', 'PatientGroup','recorded'
		 ),
...
'Label' => '記録', 'Draw' => 'timestamp')
 ...
 'ICOLS' => array ('OrderDate', 'Patient', 'SampleDate', 'urgent', 'scheduled', 'late_entry', 'comment', 'title', 'DrCode', 'Cancelled', 'test_app_type', 'printer', 'PatientGroup','recorded'),
...
WHERE O."ObjectID" = ' . mx_db_sql_quote($oid);
  ...
  if($pt_oid) {
    ...
    if($ptinfo['性別'] == '女'|| $ptinfo['性別']=='Female') {
      ;//default
    }
    elseif($ptinfo['性別'] == '男'||$ptinfo['性別'] == 'Male') {
...
';
  ...
  function print_sod($template='srl') {
    ...
    <SCRIPT LANGUAGE="JavaScript">
       window.open("print2-test.php?test_app_type=${test_app_type}&status=${status}&oid={$oid}", "","width=640,height=640");
    </SCRIPT>
HTML;
    print $s;
  }
...
SQL;
  ...
  function annotate_form_data(&$data) {
	...
	$stmt = 'select M."ObjectID" as "TestID", M."Name" as "TestName"
                   from test_master0 M , test_order_content C
	...
	$stmt = 'SELECT "TestID", "Name" as "TestName"
               FROM test_set_map S, test_master0 M
  ...
  function accept_subpick($subpick, $chosen) {
	...
	if($a[0] == 33)
      ...
      # HACK for TAKAMIYA which depends on test_master000 id 29 for Na&K, 33 for Li
...
//$cc=$aaa['Test'][$i]['Category'];
$pp= $aaa['Test'][$i]['TestName'];
//$aa= $aaa['Test'][$i]['amount'];
//$dd= $aaa['Test'][$i]['duration'];

$ocont=$ocont. $pp." :".$aa.
"  \n";
}
$ocont="指示医：".$shiji."  ". $ocont;
...
SQL;
	...
	$details = array();
	foreach($all as $e) {
	  $stmt = 'SELECT "Name" FROM test_order_content C JOIN test_master0 M ON C."TestID"=M."ObjectID" AND M."Superseded" IS NULL WHERE "TestOrder"=' . $e['ObjectID'];
