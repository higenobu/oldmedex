...
function _lib_u_test_get_doctors() {
  ...
  $stmt = <<<SQL
   ...
   AND (T."職種" = '医師')
    where "LaboSystemCode" is not null
    order by E."userid"
SQL;
...
class test_order_display extends simple_object_display {
  ...
  function dx_tableview($desc, $data, $changed) {
    ...
    $value = '<table class="listofstuff">
              <tr><th>カテゴリ</th><th>コード</th><th>検査項目</th><th>値</th><th>判定結果</th><th>コメント</th><th>正常値</th></th></tr>
...
';
	...
	$value = $value .'<tr class="'.$row_class.'"><td width="20%"></td>';
      ...
      if ($prev_item != $row['TestName'] and $row['Independent'] != 1) {
	// add item name
