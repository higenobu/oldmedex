...
include_once $_SERVER['DOCUMENT_ROOT'].'/lib/ooo_masaki.php';
...
function go_pdf($oid, $shots, $template=NULL) 
{
  ...
  $params['PRESCRIPTION_DATE'] = mx_wareki($ord['½èÊýÇ¯·îÆü'],
                                           array('dayofweek'=>1));
  $params['PRESCRIPTION_STARTDATE'] = mx_wareki($ord['½èÊý³«»ÏÆü']);
  ...
  if ($days< 0)
    $days = 0;
  $rx_end = date('Y-m-d',
		 strtotime(sprintf("%s +%d day", $ord['½èÊý³«»ÏÆü'], $days)));
  ...
  $params['BODY'] = implode("\n", set_body($meds, $shots, $ord["¸åÈ¯ÉÊ"]));

  // individual lines
  $a = set_body($meds, $shots, $ord["¸åÈ¯ÉÊ"], 1);
  for($i=0; $i < count($a); $i++)
