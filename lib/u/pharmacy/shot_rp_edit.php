...
function direction_enum($med_type=NULL) {
  ...
  $stmt = <<<SQL
    SELECT "ObjectID", "用法"
    FROM "処方箋用法"
    WHERE "Superseded" IS NULL AND (SORTORDER IS NULL OR SORTORDER >0 ) and "type"=9
...
// 
//11-3-2014  shots
 $dp_cfg = u_pharmacy_shots_order_drugpick_cfg($cfg['PatientInOut']);
 
    $this->drug_pick = new drugpick($this->drug_prefix, $dp_cfg,1);
//print_r($this->drug_pick);
