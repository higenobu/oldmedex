...
//changed 04-10-2012
class drugpick_selector extends list_of_simple_objects {

  var $default_row_per_page = 10;

  function drugpick_selector($prefix, $drugpick) {
    $this->drugpick = $drugpick;
    $cfg = array();

    $effic_code = $drugpick->effic_code;
    $injection = $drugpick->injection;
    $not_injection = $drugpick->not_injection;
 //if ($injection) print "first III";

    $oo = $drugpick->lcols;
    foreach ($drugpick->list_ids as $col)
      if (! array_key_exists($col, $oo))
	$oo[] = $col;
    $cfg['COLS'] = $drugpick->lcols;
    $cfg['ALLOW_SORT'] = $drugpick->allow_sort;
    ...
    $cfg['LLAYO'] = $layo;
    $cfg['ENABLE_QBE'] = $drugpick->enable_qbe;
    $cfg['DEFAULT_QBE'] = $drugpick->default_qbe;
    $cfg['LIST_IDS'] = $this->drugpick->list_ids;
    $cfg['SHOW_ANNOTATION'] = $drugpick->show_annotation;
  ...
  function draw_qbe_entry_form() {
    ...
    $um = mx_get_usermode($this->drugpick->config['u'], 'rx_match_exp');
    $pref = $um;
    ...
    if($pref != $um)
      mx_set_usermode($this->drugpick->config['u'], 'rx_match_exp', $pref);
...
//04-12-2012
 ...
 'Compare' => 'M."kananame"',
		     ...
		     'STRIDE' => 7,
		     'SKIP_CATEGORY' => 0,
//11-2-2014 injection =1 not_injection =0
		     'INJECTION' => 1,
		     'NOT_INJECTION' => 0,
		     'SHOW_ANNOTATION' => 1,);
}
...
class drugpick {
  ...
  function drugpick($prefix, $config=NULL,$shot=NULL) {
...
if ($shot==1)
    ...
    $this->selector = new drugpick_selector($prefix . 'sel-', $this);
...
function u_pharmacy_rx_order_drugpick_cfg($pt_outin='O', $u=0) {
...
//11-01-2014
	       'INJECTION' => 0,
//0608-2011 changed to 1 at 0327-2012
	       'NOT_INJECTION' =>1,
	       'SHOW_ANNOTATION' => 0,);
  if($_mx_show_accept) 
//0410-2012 12-2012 レセプト電算処理システム医薬品名 from 病院使用医薬品名
    $drugpick_cfg['LCOLS'] = array(array('Column' => "レセプト電算処理システム医薬品名",
					  'Label' => "医薬品"),
				    array('Column' => "当院採用",
					  'Label' => "院内採用薬",
					  'Draw' => 'enum',
...
/*
  if($pt_outin == 'O')
    $drugpick_cfg['DEFAULT_QBE'] = array($__uiconfig_u_pharmacy_outpatient_default);
...
function u_pharmacy_shots_order_drugpick_cfg($pt_outin='O', $u=0) {
  global $__uiconfig_u_pharmacy_qbe;
  global $__uiconfig_kbd;
  global $__uiconfig_rx_kbd;
  global $_mx_inc_kbd_search;
