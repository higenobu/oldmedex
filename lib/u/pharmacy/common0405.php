...
function get_pharm_order ($oid, $shots=NULL) {
  ...
  order by "ObjectID"';
    ...
    $str = 'select "ObjectID" as oid, 
                 ...
                 "日数", "区分", "定期臨時", "停止日", "停止医", "記録者", "薬剤記録者",
                 "調剤薬剤師", "調剤年月日", "看護師実施", "看護記録者",
                 "Comment", "funsai","generic","startdate"
  from  "薬剤処方箋"
  where "ObjectID" = ' . mx_db_sql_quote($oid) . ' 
  order by "ObjectID"';
...
//0730-2011
function get_xct_order ($oid ) {
  $con = mx_db_connect();
 
    $str = 'SELECT "ID", "ObjectID" as oid, "CreatedBy", orderdate, plandate, 
       ...
       drsyoken, proof, shiji, gishi, stop from "xctorder" WHERE "ObjectID"=' . mx_db_sql_quote($oid) . ' 
  order by "ObjectID"';
...
function get_curr_ord($table,$oid,$shots=NULL) {
  ...
  select "CreatedBy", "処方年月日", "処方開始日", "日数", "記録者",
         ...
         "薬剤記録者", "後発品", "Comment","generic","startdate"
  from "薬剤処方箋"
  where "ObjectID" = ' . "'{$oid}'";
...
//	break;
 ...
 print '<h1><font color="red">kouhatuhin:'. $str.'
             </font></blink></h1><br>'; 
