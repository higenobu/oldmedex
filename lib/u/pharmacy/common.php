...
//new 11-02-2014
function get_shot_order ($oid, $shots=NULL) {
...
function get_nurse_history ($pid,$ym) {
  $con = mx_db_connect();
  $str = 'select "ObjectID" as oid, "処方年月日", "区分", kubun,
         "看護師実施", "看護記録者"
  from "薬剤処方箋"
...
function get_history ($pid,$ym,$case) {
  $con = mx_db_connect();
  $str = 'select "ID" as oid, "処方年月日", "kubun", "停止日",shiji
  from "薬剤処方箋"
  where "患者" = ' . mx_db_sql_quote($pid) . ' and
        "Superseded" is NULL ';
...
function get_curr_ord($table,$oid,$shots=NULL) {
  ...
  where "ObjectID" = ' . "'{$oid}'";
    ...
    $str = '
  select "CreatedBy", "処方年月日", "処方開始日", "日数", "記録者",
         "患者", "区分", "院内院外","停止日", "停止医", "調剤薬剤師", 
         "調剤年月日", "注射", "看護師実施", "看護記録者",kubun,
         "薬剤記録者", "後発品", "Comment", "funsai",startdate,generic,shiji
  from "薬剤処方箋"
  where "ObjectID" = ' . "'{$oid}'";
 ...
 $ordkey[] = '"日数"';
  ...
  elseif ($rp)
    ...
    $str = 'insert into "薬剤処方箋" ("CreatedBy",' . implode(',',$ordkey) . 
      ") values ('{$var['u']}'," . implode(',',$ordval) . ");\n";
//0728-2012
//print $str."AAAAAAAAAAAAAAAAAAAAAAAAA";
...
function get_prescription_list() {
  ...
  $res = pg_query($con, 'select O."ObjectID" as oid,
     O."区分", O."処方年月日", O."処方開始日",o."startdate", O."患者" as pid
     from
          "薬剤処方箋" as O
     where
           O."調剤薬剤師" is NULL and
