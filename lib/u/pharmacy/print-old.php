...
function set_body($meds, $hr, $generic_ok, $tab=NULL)
{
 ...
 //   $buff[] = "------------------------------------------------------------";
  ...
  foreach ($meds as $med) {
    ...
    $LEN=70;
	if (strlen($medname) >56) {
		$medname1 = substr($medname, 0, 56);
               $medname2 = substr($medname,56,20);
 
       if($tab){
           $buff[] = sprintf("%s\t%s\t%s\t%s",$header,$medname1,$med['用量'],$unit);
	$buff[] = sprintf("%s\t%s",'  ',$medname2);
		}     
	else {
           $buff[] = sprintf("%2s %s %2s %s",$header,$medname1,$med['用量'],$unit);
	$buff[] = sprintf("%2s %s",'  ',$medname2);
   	}
}
else{
       ...
       if($tab)
           ...
           $buff[] = sprintf("%2s %s %s%s",$header,$medname,$med['用量'],$unit);
    }
