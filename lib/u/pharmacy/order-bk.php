			   ...
			   'DCOLS' => array('処方年月日',
					    ...
					    array('Column' => '定期臨時',
						  ...
						  'Enum' => array(1 => '定期',
								  2 => '臨時')),
					    '停止日',
					    array('Column' =>'停止医',array('Column' => 'DETAILS',
						  'Label' => '処方内容',
						  'Draw' => 'order_detail'
						  ),
						  'Draw' => 'employee'),
...
//$cfg['L_RANDOM_LAYOUT']=$cfg['D_RANDOM_LAYOUT'];
...
$stmt = <<<SQL
SELECT C."ObjectID", C."RPID",
    C."薬剤" , M."レセプト電算処理システム医薬品名" as yakuzaimei, M."包装単位単位",
    C."用量", 
    C."用法", Y."用法" as "用法名称", 
    C."日数", C."用法分類", C."その他コメント"
FROM "薬剤処方箋内容" as C
...
$val = array();
	...
	if (!$data || !count($data)) {
		...
		foreach ($data as $d) {
			$items=$items.$d['yakuzaimei'];
			
			
		}

	}
		$d['DETAILS'] = implode(', ', $items);
