...
//11-08-2014 added kartenew
 
'rx_module_index_info',
'kartenew_module_index_info',
...
'injection_module_index_info',
 'old_injection_module_index_info',
...
function order_cmp($data) {
 
//0606-2014 order list sort disease must be before karte
foreach ($data as $key => $row) {
    $timestamp[$key]  = $row['timestamp'];
    $type[$key] = $row['type'];
$obj[$key] = $row['object_id'];
if (substr($type[$key],0,2)=='qu') $type[$key]='wquick';
if (substr($type[$key],0,2)=='di') $type[$key]='ldisease';
//print $timestamp[$key]. $type[$key];
}
  array_multisort($timestamp, SORT_DESC,$type, SORT_DESC, $obj, SORT_DESC,$data);
//print_r($data);
return $data;
}
...
// $a[]=$uid["ID"];
//0722-2012 shiji
//print 'AAAAAAAAAAAAAAAAAAA'.$u.'login-user';
...
//04-04-2012
	$stmt = <<<SQL
		...
		WHERE P."Superseded" IS NULL AND y."Superseded" IS NULL
		$group_limit
		$a
SQL;
//0722-2012
...
//   print "call from index-pt-app".$oid."|".$pid."|".$date_from."|".$date_to;  
  $me = $_SERVER['PHP_SELF'];
  $match = array();
...
//0530-2014 keep this usort
// usort($result, 'order_cmp');
// usort($result, 'timestamp_cmp');
$result=order_cmp($result);
//print_r($result);
...
<script  type=\"text/javascript\">
    function winOpen9(){
        mywindow9=window.open(".
  $cookie.
		     $d['callback_url'].
		    $d['timestamp'].');}'.
 '</script>';

print '<a href="javascript:void(0)" onclick="winOpen9();"> Open9 </a>';
 
//print '<a href="javascript:closeWin();"> Close  </a>
print '<br />';
*/
...
//06-11-2014
	      ...
	      printf("<a href=\"%s%s\" target=\"_blank\" >%s</a>",
		     $cookie,
