...
$_lib_u_ct_record_undesirable2_base_stmt = 'select "ObjectID" from "¼£¸³Áí³ç2"';

$_lib_u_ct_record_undesirable2_cfg = array
(
 'TABLE' => '¼£¸³Áí³ç2',
 'COLS' => array('¼£¸³¥ª¡¼¥À'),
 'ECOLS' => array('¼£¸³¥ª¡¼¥À'),
 'ICOLS' => array('¼£¸³¥ª¡¼¥À'),
 'HSTMT' => $_lib_u_ct_record_undesirable2_base_stmt .' WHERE (NULL IS NULL) ',
 'STMT' => ($_lib_u_ct_record_undesirable2_base_stmt .' WHERE  ("Superseded" IS NULL) '),
 );

 //Áí³ç
 $_lib_u_ct_record_undesirable2_ecols = array
 (
  0 => "(¾É¾õ 1)",
  1 => "(¾É¾õ 2)",
  2 => "(¾É¾õ 3)",
 ...
 $_lib_u_ct_record_undesirable2_ecols_template = array(
					  array("Column" => "»ö¾ÝÌ¾",
						'Draw' => 'text',
...
function _lib_u_ct_record_undesirable2_fetch_data($it, $oid) {
  global $_lib_u_ct_record_undesirable2_cfg;
  global $_lib_u_ct_record_undesirable2_ecols;
  global $_lib_u_ct_record_undesirable2_ecols_template;

  $db = mx_db_connect();

  // Fetch from the main table.
  $stmt = ($_lib_u_ct_record_undesirable2_cfg['HSTMT'] .
	   'AND "ObjectID" = ' . mx_db_sql_quote($oid));
  $data = pg_fetch_all(pg_query($db, $stmt));
  $data = $data[0];
  // Fetch from subtables.
  $stmt = 'SELECT * FROM "¼£¸³Áí³ç2¥Ç¡¼¥¿" WHERE "¼£¸³Áí³ç2"=' .
    mx_db_sql_quote($oid);
  ...
  if (is_array($d)) {
	...
	list($row['Å¾µ¢³ÎÇ§Æü'],$row['Å¾µ¢³ÎÇ§»þ¹ï']) = explode(' ', $row['Å¾µ¢³ÎÇ§Æü»þ']);
      ...
      $base = $_lib_u_ct_record_undesirable2_ecols[$i++];
      foreach($_lib_u_ct_record_undesirable2_ecols_template as $k => $v) {
...
class ct_record_undesirable2_edit extends simple_object_edit {
  var $debug = 1;
  function ct_record_undesirable2_edit($prefix, &$app, $cfg=NULL) {
    global $_lib_u_ct_record_undesirable2_cfg;
    global $_lib_u_ct_record_undesirable2_ecols;
    global $_lib_u_ct_record_undesirable2_ecols_template;
    if (is_null($cfg))
      $cfg =& $_lib_u_ct_record_undesirable2_cfg;
    _lib_u_ct_annotate_cfg(&$cfg, $_lib_u_ct_record_undesirable2_ecols,
			   $_lib_u_ct_record_undesirable2_ecols_template);
    $this->app = $app;
  ...
  function edit($chiken_id) {
    ...
    // ID here means ¼£¸³¥ª¡¼¥À id.  What I really want is ¼£¸³Áí³ç2 id.
    $stmt = 'select "ObjectID" from "¼£¸³Áí³ç2" where "Superseded" is NULL and "¼£¸³¥ª¡¼¥À"=' . $chiken_id;
    //print $stmt;
  ...
  function fetch_data($id) {
    $d = _lib_u_ct_record_undesirable2_fetch_data($this, $id);
  ...
  function _draw_header() {
    // draw header
    print "<thead>";
    print "<tr><th colspan=10>Í­³²»ö¾Ý(Î×¾²¸¡ººÃÍ)Áí³ç</th></tr>";
  ...
  function _draw_body($desc, $name, $value) {
    ...
    foreach($_lib_u_ct_record_undesirable2_ecols as $k => $v) {
      # 0 => shoujyou 1
      # 1 => shoujyou 2...
      if(!$first) {
	$first = false;
	...
	$this->_draw_body3($this->data, $ec, $epages, $span);
	print "</tr>\n";
	if($count < count($_lib_u_ct_record_undesirable2_ecols) - 1) 
	  print "<tr>";
	$count += 1;
      }
    }
