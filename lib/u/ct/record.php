...
$_lib_u_ct_record_base_stmt = 'select "ObjectID" from "治験変動判定"';

$_lib_u_ct_record_cfg = array
(
 'TABLE' => '治験変動判定',
 'COLS' => array('治験オーダ'),
 'ECOLS' => array('治験オーダ'),
 'ICOLS' => array('治験オーダ'),
 'HSTMT' => $_lib_u_ct_record_base_stmt .' WHERE (NULL IS NULL) ',
 'STMT' => ($_lib_u_ct_record_base_stmt .' WHERE  ("Superseded" IS NULL) '),
 );

$_lib_u_ct_record_ecols = array
(
 "血液学的検査" => array("白血球数",
			 "赤血球数",
			 "ヘモグロビン量",
 ...
 $_lib_u_ct_record_ecols_template = array(
					  array('Column' => "異常の有無",
						'Draw' => "radio",
						'Enum' => array(NULL => '未判定',
								'1' => 'なし',
...
function copy_ecols2(&$cfg, $col) {
  global $_lib_u_ct_record_ecols_template;
  foreach($_lib_u_ct_record_ecols_template as $k => $v) {
...
function _lib_u_ct_annotate_cfg(&$cfg) {
  global $_lib_u_ct_record_ecols;
  
  $cfg['ECOLS'] = array();
  //大項目（必ずarray）
  foreach($_lib_u_ct_record_ecols as $dk => $dv)
    foreach($dv as $ck => $cv)
      if(is_array($cv))
	foreach($cv as $sk => $sv)
...
function _lib_u_ct_record_fetch_data($it, $oid) {
  global $_lib_u_ct_record_cfg;
  global $_lib_u_ct_record_ecols_template;

  $db = mx_db_connect();

  // Fetch from the main table.
  $stmt = ($_lib_u_ct_record_cfg['HSTMT'] .
...
class ct_record_edit extends simple_object_edit {
  var $debug = 1;
  function ct_record_edit($prefix, &$app, $cfg=NULL) {
    global $_lib_u_ct_record_cfg;
    if (is_null($cfg))
      $cfg =& $_lib_u_ct_record_cfg;
  ...
  function fetch_data($id) {
    $d = _lib_u_ct_record_fetch_data($this, $id);
  ...
  function _draw_body($desc, $name, $value) {
    global $_lib_u_ct_record_ecols;
    $first = true;
    foreach($_lib_u_ct_record_ecols as $k => $v) {
      if(!$first) {
	$first = false;
	print "</tr>";
      }
      print "<tr>";
      printf("<td rowspan=%d>%s</td>", count_elements($v),
	     mx_vstring($k));
      $count = 0;
      foreach($v as $k2 => $v2) {
	if(is_array($v2)) {
