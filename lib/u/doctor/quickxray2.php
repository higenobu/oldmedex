...
$stmt = 'SELECT M.name AS "部位名称",M."ObjectID" AS "部位ObjectID",D.dirs, D.leftdir, D.rightdir,D.bothdirs, D.digits , O.shiji, O.yotei,O.mokuteki,O.stop  FROM "ctorder" AS O JOIN "ctordercont" AS D ON O."ObjectID" = D."Ｘ線オーダ" JOIN "ctmaster" AS M ON M."ObjectID" = D."部位" AND M."Superseded" IS NULL WHERE O."ObjectID" = ' . $oid;
...
// 0325-2011
	...
	/* foreach ($data as $e) {
		...
		if (array_key_exists('B', $r) && $e['bothdirs'])
			$val["$name.B"] = 'Y';
		for ($i = 0; $i < strlen($e['digits']); $i++) {Ｘ線オーダ
			$it = "D" . $e['digits'][$i];
			$d = "$name.$it";
			if (array_key_exists($it, $r))
				$val[$d] = 'Y';
 ...
 */
function quickxray2_module_index_info
...
//0710-2014
		$object = $e['ObjectID'];
		$config = array();
		$sod = new quickxray2_display("sod-$ix-", &$config);
		$sod->reset($object);
