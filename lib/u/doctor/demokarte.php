...
function __lib_u_doctor_quickxray2_cfg(&$cfg)
{
	...
	$lcols = $static_cols;
	$lcols[] = array('Column' => 'P');

	$local_cfg = array('TABLE' => 'karte',
			   'LCOLS' => $lcols);
...
(&$dbh, $p_oid, $p_pid, $time_from, $time_to, $options=NULL) {
	if (!is_null($time_from))
		$limit[] = 'X."orderdate" >= '. mx_db_sql_quote($time_from);
	if (!is_null($time_to))
		$limit[] = 'X."orderdate" <= '. mx_db_sql_quote($time_to);
	if (!count($limit))
		$num_limit = 1;
	else
		$num_limit = 0;
