...
include_once $_SERVER['DOCUMENT_ROOT'].'/lib/u/doctor/pdf60.php';
...
function __lib_u_formchk_cfg(&$cfg)
{
...
$cfg = array_merge(
 ...
 'COLS' => array(
 	...
 	"orderdate" ,
  	...
  	"´µ¼Ô" ,
	 plandate, 
       procdate,  teikikubun, xctkubun, techname, techid, a1, a2, 
        b1, a5
	      ),
...
'LCOLS' => array(
	...
	array('Column' => 'orderdate','Label'=>'DOS'),
	 ...
	 array( 'Column'=>'a2','Label'=>'template','Draw'=>'enum',
'Enum'=>array('1'=>'History',
'2'=>'UGI','3'=>'CXR',
),),
	array( 'Column'=>'a1','Label'=>'Package','Draw'=>'enum',
'Enum'=>array('1'=>'A',
'2'=>'B',
),),       
...
class list_of_formchks extends list_of_ppa_objects {
	function list_of_formchks($prefix, &$cfg) {
		if (is_null($cfg))
			$cfg = array();
		$cfg = __lib_u_formchk_cfg($cfg);
...
class formchk_display extends simple_object_display {
	
function formchk_display($prefix, &$cfg) {
		if (is_null($cfg))
			$cfg = array();
		$cfg = __lib_u_formchk_cfg($cfg);
...
class formchk_edit extends simple_object_edit {
	function formchk_edit($prefix, &$cfg) {
		if (is_null($cfg))
			$cfg = array();
		$cfg = __lib_u_formchk_cfg($cfg);
