...
function __lib_u_doctor_quickxray_schema()
{
	global $__lib_u_doctor_quickxray_schemainfo;
	if (!$__lib_u_doctor_quickxray_schemainfo) {
		$db = mx_db_connect();
		$stmt = <<<SQL
...
SQL;
		...
		$__lib_u_doctor_quickxray_schemainfo =
			array('XCOLS' => $cols, 'REVINDEX' => $revinfo);
	}
	return $__lib_u_doctor_quickxray_schemainfo;
}

function __lib_u_doctor_quickxray_cfg(&$cfg)
{
	$schema = __lib_u_doctor_quickxray_schema();
	$dynamic_cols = $schema['XCOLS'];
	$static_cols = array(
		array('Column' => '日付', 'Draw' => 'date',
		      'Option' => array('validate' => 'nonnull,date')),
		array('Column' => 'その他', 'Draw' => 'textarea'),

		array('Column' => 'yotei','Label' => '予定日',
			'Draw' => 'date',
			'Option' => array('validate' => 'date', 'list' => 1)
			),
		array('Column' => 'shiji',
					'Label' => '指示医',
	...
	$lcols = $static_cols;
	$lcols[] = array('Column' => '部位方向');

	$local_cfg = array('TABLE' => 'Ｘ線オーダ',
			   'LCOLS' => $lcols
...
function __lib_u_doctor_quickxray_fetch_sub(&$db, $id)
{
	$idq = mx_db_sql_quote($id);
	$stmt = <<<SQL
SELECT
M.name AS "部位名称",
M."ObjectID" AS "部位ObjectID",
O.yotei as "yotei",
O.shiji as "shiji",
O.stop as "stop",
...
FROM "Ｘ線オーダ" AS O
JOIN "Ｘ線オーダ内容" AS D
ON O."ObjectID" = D."Ｘ線オーダ"
JOIN "Ｘ線部位マスタ" AS M
ON M."ObjectID" = D."部位" AND M."Superseded" IS NULL
...
function __lib_u_doctor_quickxray_apply_template(&$data, $it, $read_only=NULL)
{
	$schema = __lib_u_doctor_quickxray_schema();
	$revinfo = $schema['REVINDEX'];
	$s = file_get_contents($_SERVER['DOCUMENT_ROOT'] .
			       '/templates/quickxray.html');
	$result = array();
	while ($s != '') {
		if ($read_only) {
...
function __lib_u_doctor_quickxray_apply_template1(&$data, $it, $read_only=NULL)
{
	$schema = __lib_u_doctor_quickxray_schema();
	$revinfo = $schema['REVINDEX'];
	$s = file_get_contents($_SERVER['DOCUMENT_ROOT'] .
			       '/templates/quickxray1.html');
	$result = array();
	while ($s != '') {
		if ($read_only) {
			$s = str_replace("<input type",
...
function __lib_u_doctor_quickxray_summarize($dcols, $d)
{
	$items = array();
...
class list_of_quickxrays extends list_of_ppa_objects {
	function list_of_quickxrays($prefix, $config=NULL) {
		__lib_u_doctor_quickxray_cfg(&$config);
	...
	function annotate_fetched_data() {
		...
		foreach ($this->fetched_data as $d) {
			$oid = $d['ObjectID'];
			$v = __lib_u_doctor_quickxray_fetch_sub(&$db, $oid);
			$items = __lib_u_doctor_quickxray_summarize($dc, $v);
			$d['部位方向'] = implode(', ', $items);
			$annotated[] = $d;
		}
...
class quickxray_display extends simple_object_display {
	function quickxray_display($prefix, $config=NULL) {
		__lib_u_doctor_quickxray_cfg(&$config);
	...
	function annotate_row_data(&$data) {
		$db = mx_db_connect();
		$val = __lib_u_doctor_quickxray_fetch_sub
			(&$db, $data['ObjectID']);
		$data = array_merge($data, $val);
	}

	function draw_body($data, $hdata) {
		print __lib_u_doctor_quickxray_apply_template1(&$data, $this, 1);
	}

	function module_info($p_pid) {
		$d = $this->prepare_data_for_draw();

		$candidates = array("u/doctor/quickxray.php",
				    "u/everybody/quickxray.php");
...
$url = sprintf("/$application?SetPatient=1&".
		...
		$items = __lib_u_doctor_quickxray_summarize($dcols, $d);
  ...
  */
  ...
  $status = 0;
    ...
    <SCRIPT LANGUAGE="JavaScript">
       window.open("print20.php?test_app_type=${test_app_type}&status=${status}&oid={$oid}", "","width=640,height=640");
    </SCRIPT>
HTML;
    print $s;
...
class quickxray_edit extends simple_object_ppa_edit {
	function quickxray_edit($prefix, $config=NULL) {
		__lib_u_doctor_quickxray_cfg(&$config);
...
SQL;
		...
		foreach ($revinfo as $name => $pieces) {
			...
			$stmt = <<<SQL
INSERT INTO "Ｘ線オーダ内容"
("Ｘ線オーダ", "部位", "dirs", "leftdir", "rightdir", "bothdirs", "digits")
SELECT
...
SQL;
	...
	function draw_body_1($data, $soc) {
		print __lib_u_doctor_quickxray_apply_template(&$data, $this);
	}
 ...
 */
function quickxray_module_index_info
(&$dbh, $p_oid, $p_pid, $time_from, $time_to, $options=NULL) {
	if (!is_null($time_from))
		$limit[] = 'X."日付" >= '. mx_db_sql_quote($time_from);
	if (!is_null($time_to))
		$limit[] = 'X."日付" <= '. mx_db_sql_quote($time_to);
	if (!count($limit))
		$num_limit = 1;
	else
		$num_limit = 0;
...
SQL;
	...
	foreach ($all as $e) {
		$object = $e['ObjectID'];
		$config = array();
		$sod = new quickxray_display("sod-$ix-", &$config);
		$sod->reset($object);
