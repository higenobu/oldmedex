...
/* for debug comment out 0715-2014
...
//03-13-2011 debug 
    $str = 'select "ID", "ObjectID" , "Superseded", "CreatedBy", orderdate, plandate, 
...
function get_pharm_order ($oid, $shots=NULL) {
  ...
  order by "ObjectID"';
    ...
    $str = 'select "ObjectID" as oid, 
                 ...
                 "日数", "区分","院内院外", "定期臨時", "停止日", "停止医", "記録者", "薬剤記録者",
                 "調剤薬剤師", "調剤年月日", "看護師実施", "看護記録者",kubun,
                 "Comment", "funsai",shiji,generic, startdate
  from  "薬剤処方箋"
  where "ObjectID" = ' . mx_db_sql_quote($oid) . ' 
  order by "ObjectID"';
...
function get_xct_order ($oid, $shots=NULL) {
  $con = mx_db_connect();
...
function get_k_order ($oid, $shots=NULL) {
  $con = mx_db_connect();
  
//01-01-2014
    $str = 'SELECT "ID", "ObjectID", "Superseded", "CreatedBy", orderdate, copysign
  FROM korder1
  where "ObjectID" = ' . mx_db_sql_quote($oid) . ' 
  order by "ObjectID" desc';
  return pg_fetch_assoc(pg_query($con,$str));
...
//03-01-2014
function get_formchk ($oid, $shots=NULL) {
  $con = mx_db_connect();
...
//03-13-2011
    $str = 'select "ObjectID" as oid, 
                 orderdate, plandate,  "患者", xctkubun, teikikubun, bui1, memo1, memo11
