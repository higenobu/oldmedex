  ...
  $cfg = array_merge
    ($cfg, array
     ('TABLE' => 'リハ処方箋',
      'ALLOW_SORT' => 1,
      'DEFAULT_SORT' => '日付',
      'LCOLS' => array('リハ箋ID', '日付', '処方医', '理', '作', '言'),

      'UNIQ_ID' => 'X."ObjectID"',
      ));
  
  $stmt_head = '
SELECT X."ObjectID" as "リハ箋ID",
       X."処方日" as "日付",
       (DR."姓" || DR."名") as "処方医",
...
class list_of_rehab_norders extends list_of_simple_objects {

  var $default_page_direction = 'U';
  var $debug = 1;
  ...
  function base_fetch_stmt() {
    return (list_of_simple_objects::base_fetch_stmt() .
	    ' AND "患者" = ' .
	...
	__lib_u_therapist_order_cfg_detailed_flippage
		(&$cfg, '内容2', array
		 (array('Label' => '失語症訓練',
		  ...
		  array('Label' => '実用的なコミュニケーションに関する検査',
			'Draw' => 'check_box',
			'Column' => array("CADL")),
		  array('Label' => '全失語',
			'Draw' => 'check_box',
