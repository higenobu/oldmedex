...
function __lib_u_therapist_order_cfg(&$cfg) {
  $cfg = array_merge
    ($cfg, array
     ('TABLE' => 'リハ処方箋',
      'ALLOW_SORT' => array
...
class list_of_rehab_orders extends list_of_ppa_objects {

  var $default_row_per_page = 4;
  var $debug = 0;

  function list_of_rehab_orders($prefix, $cfg=NULL) {
    if (is_null($cfg)) $cfg = array();
    __lib_u_therapist_order_cfg(&$cfg);
...
$__lib_u_therapist_order_yucky_cols = array
(
"関節可動域訓練PT",
"関節可動域訓練OT",
"筋力増強訓練PT",
...
function __lib_u_therapist_order_cfg_detailed_flippage(&$cfg, $name, $ary)
{
...
function __lib_u_therapist_order_cfg_detailed(&$cfg) {
	global $__lib_u_therapist_order_yucky_cols;

	__lib_u_therapist_order_cfg(&$cfg);

	__lib_u_therapist_order_cfg_detailed_flippage
		(&$cfg, 'リハ箋', array
		 ('日付',
		  '記録日',
		  '処方医',
	...
	__lib_u_therapist_order_cfg_detailed_flippage
		(&$cfg, '機能障害1', array
		 ("意識障害",
		  array('Option' => array('OmitIfEmpty' => 1),
			'Column' => "意識障害コメント"),
		  "JCS",
		  array('Column' => "痴呆",
			'Label' => "痴呆(見当識障害)"),
	...
	foreach ($__lib_u_therapist_order_yucky_cols as $c) {
		$sel[] = "X.\"$c\" AS \"$c\"";
		$cfg['AlreadyHasSelectionFor'][$c] = 1;
	}
...
class rehab_order_display extends simple_object_display {

  var $debug = 0;

  function rehab_order_display($prefix, $cfg=NULL) {
    if (is_null($cfg)) $cfg = array();
    __lib_u_therapist_order_cfg_detailed(&$cfg);
