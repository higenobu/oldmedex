...
include_once $_SERVER['DOCUMENT_ROOT'].'/lib/u/therapist/order.php';

$_lib_u_therapist_execution_code_enum = array
(
 '180027410' => '心大血管疾患リハ（１）',
 '180027510' => '心大血管疾患リハ（２）',
 '180027610' => '脳血管疾患等リハ（１）',
 '180027710' => '脳血管疾患等リハ（２）',
 '180027810' => '運動器リハ（１）',
 '180027910' => '運動器リハ（２）',
 '180028010' => '呼吸器リハ（１）',
 '180028110' => '呼吸器リハ（２）',
 ...
 '180022110' =>
   array('old' => 1, 'value' => '理学療法（Ⅰ）個別'),
 '180022210' =>
   array('old' => 1, 'value' => '理学療法（Ⅰ）集団'),
 '180023610' =>
   array('old' => 1, 'value' => '作業療法（Ⅰ）個別'),
 '180023710' =>
   array('old' => 1, 'value' => '作業療法（Ⅰ）集団'),
 '180024810' =>
   array('old' => 1, 'value' => '言語聴覚療法（Ⅰ）個別'),
 '180024910' =>
   array('old' => 1, 'value' => '言語聴覚療法（Ⅰ）集団'),
);

function __lib_u_therapist_exec_list_cfg(&$cfg) {
  $cfg = array_merge
    ($cfg, array
     ('TABLE' => 'リハ実施記録',
      'ALLOW_SORT' => array
...
class list_of_rehab_execs extends list_of_simple_objects {

  var $default_row_per_page = 4;
  var $debug = 0;

  function list_of_rehab_execs($prefix, $cfg=NULL) {
    if (is_null($cfg)) $cfg = array();
    $this->loo = new list_of_rehab_orders($prefix . 'loo-', $cfg);
    $this->sod = new rehab_order_display($prefix . 'sod-', $cfg);
    ...
    __lib_u_therapist_exec_list_cfg(&$cfg);
...
function __lib_u_therapist_exec_cfg(&$cfg) {
	__lib_u_therapist_exec_list_cfg(&$cfg);
...
function __lib_u_therapist_exec_fetch_subs($id, &$data)
{
	$stmt = ('SELECT 
...
function __lib_u_therapist_exec_unannotate_datetime($data, $v)
{
	/* Reverse annotation */
	$match = array();
	if (preg_match('/^\d+:\d+(?::\d+)?$/',
...
function __lib_u_therapist_exec_annotate_datetime(&$data, $auto_end=NULL)
{
	$this_date = $data['日付'];
	$last_i = __lib_u_therapist_exec_count_subpage($data);
	for ($i = 1; $i <= $last_i; $i++) {
		if ($auto_end &&
		    $data["$i-開始日時"] != '' &&
		    0 < $data["$i-単位数"] &&
...
function __lib_u_therapist_exec_tweak_cfg(&$acols, &$apages, $fm, $to)
{
...
function __lib_u_therapist_exec_count_subpage($data) {
	$last_i = 0;
	for ($i = 1;
...
class rehab_exec_display extends simple_object_display {

	var $debug = 0;

	function rehab_exec_display($prefix, $cfg=NULL) {
		if (is_null($cfg)) $cfg = array();
		__lib_u_therapist_exec_cfg(&$cfg);
	...
	function pre_draw_hook($data, $hdata) {
		$last_i = __lib_u_therapist_exec_count_subpage($data);
		if ($hdata) {
			$ii = __lib_u_therapist_exec_count_subpage($hdata);
			if ($last_i < $ii)
				$last_i = $ii;
		}
		...
		__lib_u_therapist_exec_tweak_cfg(&$dcols,
						  &$dpages,
						  1, $last_i);
		$cfg['DCOLS'] = $dcols;
		$cfg['DPAGES'] = $dpages;
	...
	function fetch_data($id) {
		...
		return __lib_u_therapist_exec_fetch_subs($id, &$data);
	}

	function annotate_row_data(&$data) {
		__lib_u_therapist_exec_annotate_datetime(&$data);
	}

}

class rehab_exec_edit extends simple_object_edit {

	var $debug = 0;

	function rehab_exec_edit($prefix, $cfg=NULL) {
		if (is_null($cfg)) $cfg = array();
		__lib_u_therapist_exec_cfg(&$cfg);
	...
	function get_form_data() {
		...
		while (1) {
			...
			if (!array_key_exists($field, $_REQUEST))
				break;
			__lib_u_therapist_exec_tweak_cfg(&$ecols,
							  &$epages,
							  $i, $i);
			$i++;
		}
	...
	function add_empty_subpage_as_needed() {
		$last_i = __lib_u_therapist_exec_count_subpage($this->data);
		$cfg =& $this->so_config;
		...
		# Add begintime to the new page.
		$now = time();
		$begintime = strftime('%H:%M', $now - 1200);
		$this->data["$last_i-開始日時"] = $begintime;
		$cfg['ECOLS'] = $ecols;
		$cfg['EPAGES'] = $epages;
	}
	...
	function anew_edit_tweak() {
		...
		$ecols = $cfg['ECOLS_BASE'];
		$epages = $cfg['EPAGES_BASE'];
		__lib_u_therapist_exec_tweak_cfg(&$ecols, &$epages,
						  1, $last_i);
		$cfg['ECOLS'] = $ecols;
		$cfg['EPAGES'] = $epages;
	...
	function annotate_form_data(&$data) {
		...
		__lib_u_therapist_exec_annotate_datetime(&$data, 1);
	}
	...
	function _validate($force=NULL) {
		...
		for ($i = 1;
			...
			foreach (array('開始日時', '終了日時') as $fld) {
				$v = __lib_u_therapist_exec_unannotate_datetime($d, $d["$i-$fld"]);
			...
			if (!$time_unit_err) {
				$v = __lib_u_therapist_exec_unannotate_datetime($d, $d["$i-開始日時"]);
				$begin = mx_datetime_to_unixtime($v);
				$v = __lib_u_therapist_exec_unannotate_datetime($d, $d["$i-終了日時"]);
	...
	function data_compare($curr, $data) {
		...
		$last_c = __lib_u_therapist_exec_count_subpage($curr);
		while (0 < $last_c) {
			if (!$this->is_empty_subpage($last_c, $curr))
				break;
			$last_c--;
		}
		$last_d = __lib_u_therapist_exec_count_subpage($data);
		while (0 < $last_d) {
...
WHERE "リハ実施記録" = ' . mx_db_sql_quote($id));
		...
		while (1) {
			...
			$time_begin = __lib_u_therapist_exec_unannotate_datetime($this->data, $this->data["$i-開始日時"]);
			$time_end = __lib_u_therapist_exec_unannotate_datetime($this->data, $this->data["$i-終了日時"]);
			$stmt = ('
