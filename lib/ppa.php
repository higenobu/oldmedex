...
//11-13-2012 exclude done no ato AND (C."CreatedBy" IS NULL OR (C."CreatedBy" = 1 AND C."予約時刻" >= (CURRENT_DATE + INTERVAL '1 DAY')))
...
//1120-2012 do not display kanjya sentaku
$appname = $this->application_name();
//if appname second char= %m then passing patientID
 
if (substr($appname,1,1)=='%') {
 print "<table width=\"100%\"><tr valign=\"top\"><td width=\"30%\">";
//1120-2012 passing patientID to display
...
//1023-2014 delete comment next 3 lines
 //	$this->draw_ppa_applist();
...
//1023-2014 next 2 lines  show appbar fro all application
      if ($_mx_use_appbar)
 	    mx_appbar($this);
...
//11-15-2012 add target="_blank" to <a 06-11=-2014
	function top_pane() {
		$appname = $this->application_name();
		$up = $this->up();
		if ($up)
			print "<a ";
			print " href=\"$up\" target=\"_blank\" >";
