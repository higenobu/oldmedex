...
function get_meds2 ($oid,$injection) {
  $con = mx_db_connect();
  
  
    $str = 'select C."ObjectID" as medid, C."orderdate", C."procdate",
                 C."memo1" as shape, C."bui1" as medis,
                 C."memo11" as unit, C."memo12" as freq,
                 C."memo2", C."techsyoken", 
                 C."drsyoken", 
	    FROM "xctorder" as C
	    
	    WHERE C."ID" =  ' . mx_db_sql_quote($oid) . '
	    ORDER BY C."ObjectID"';
...
function print_diff($table,$array,$page, $shots=NULL) {
  ...
  foreach ($array[$page-1] as $key => $newval) {
	...
	if ($oldval) $oldval = "済";
		...
		$key == "調剤薬剤師" || $key == "看護記録者" ||
		$key == "検査師") {
	$newname = get_emp_name($newval);get_
...
function list_med($varname,$def,$type) {
  ...
  case "freq" : $str = 'select "ObjectID", "用法" as val from "処方箋用法"
			where "Superseded" is NULL and (sortorder is null or sortorder >0 ) order by sortorder';
    break;
