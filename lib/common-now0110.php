...
//1121-2013 for osato fro lcm change mx_today
//
...
$_mx_logo_url = '';
//0430-2012 
$_mx_authenticate_session_update=60;
//
...
foreach (explode(',', $_mx_apptment_days_string) as $_i) {
	switch ($_i) {
	case 0: $_mx_appt_days[$_i] = 'Sun'; break;
	case 1: $_mx_appt_days[$_i] = 'Mon'; break;
	case 2: $_mx_appt_days[$_i] = 'Tue'; break;
	case 3: $_mx_appt_days[$_i] = 'Wed'; break;
	case 4: $_mx_appt_days[$_i] = 'Thu'; break;
	case 5: $_mx_appt_days[$_i] = 'Fri'; break;
	case 6: $_mx_appt_days[$_i] = 'Sat'; break;
	}
}
$_mx_daysoftheweek = array('Mon', 'Tue', 'Wen', 'Thu', 'Fri', 'Sat', 'Sun');
...
//0922-2013
  ...
  $match = array();
  if (! preg_match('/^(\d+)\/(\d+)\/(\d{4})$/', $date, &$match) ||
      ! checkdate($match[1], $match[2], $match[3]))
...
// $_mx_authenticate_session_update = 1800; in seconds. 0328-2012
...
//0603-2013 more than 5 times login error will lock to login process.
//0110-2014
echo 'LOGIN  ERROR: Please type correct id/password'.'<br>';
$stmt1 = "select count from mx_count WHERE username = '$username'";
//print $stmt1;
$result1 = mx_db_fetch_single($db, $stmt1);
$logcount= $result1['count'];
//print $logcount;
...
if ($logcount >0){
	...
	if ($logcount < 5){
	$logcount++;
	$stmt3 ="update mx_count set count=$logcount";
	//print $stmt3;

	if (pg_query($db, $stmt3))
		    pg_query($db, 'commit');
			}

	else {
		print "exceeded login error";
		}
}
	...
	$stmt2 = "insert into  mx_count(count, username) VALUES (1,'$username')";
	//print $stmt2;
...
//0920-2013
function mx_calc_age1($birth, $asof=NULL)
{
    
if(is_null($asof))
        $asof = date("m/d/Y");
    list($tm, $td,$ty) = explode('/', $asof);
...
//1010-2012 mailing function
function mx_smp($mailaddr,$mailsubject, $mailcont)
{
...
$mailer->Body     = mb_convert_encoding($mailcont,"JIS","UTF-8");
