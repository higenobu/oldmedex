...
$_mx_resource_dir = 'resource/8a783773';
$_mx_logo_url = '';
...
function mx_authenticate_user($do_not_redirect=0) {
    ...
    if ($cookie) {
	$db = mx_db_connect();
/* 03222011 for expire time as 24 hours=14400 seconds */


	$stmt = ('SELECT S."ID", S.userid, S.last_check, 
                  
                    browser_hint, remote_addr' .
		 ' FROM mx_session AS S  WHERE ' .
...
$_mx_authenticate_session_update = 600; // in seconds.
...
function mx_kick_claim_if_by_pid($db, $patient_id, $date, $urgent=0)
{
...
function mx_kick_claim_if_by_poid($db, $patient_objectid, $date, $urgent=0)
{
	if (!$urgent && (mx_today_string() != $date)) {
		print "<!-- $date is not ";
		print mx_today_string();
