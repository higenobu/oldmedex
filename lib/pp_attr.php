...
function _lib_pp_attr_fetch_data($id, $attr) {
	$db = mx_db_connect();
	$substmt = '
SELECT M."¥°¥ë¡¼¥×", M."Ì¾¾Î", D."Â°À­ÃÍ", D."´µ¼ÔÂ°À­", A."µ­Ï¿Æü", A."´µ¼Ô",
       A."ID", A."ObjectID", A."Superseded", A."CreatedBy"
FROM "´µ¼ÔÂ°À­" AS A
JOIN "´µ¼ÔÂ°À­¥Ç¡¼¥¿" AS D ON D."´µ¼ÔÂ°À­" = A."ObjectID"
...
class pp_attr_soe extends simple_object_edit {
	...
	function _update_subtables(&$db, $ObjectID, $StashID) {
		...
		if (!is_null($StashID)) {
			 ...
			 */
			$stmt = '
INSERT INTO "´µ¼ÔÂ°À­¥Ç¡¼¥¿" ("´µ¼ÔÂ°À­", "Â°À­", "Â°À­ÃÍ")
SELECT ' . mx_db_sql_quote($StashID) . ', "Â°À­", "Â°À­ÃÍ"
FROM "´µ¼ÔÂ°À­¥Ç¡¼¥¿"
WHERE "´µ¼ÔÂ°À­" = ' . mx_db_sql_quote($ObjectID);
			if (! pg_query($db, $stmt))
...
AND "Â°À­" IN (' . implode(', ', array_keys($this->attr)) . ')';
		...
		foreach ($this->attr as $attr_id => $attr_name) {
			$stmt = '
INSERT INTO "´µ¼ÔÂ°À­¥Ç¡¼¥¿" ("Â°À­", "Â°À­ÃÍ", "´µ¼ÔÂ°À­")
