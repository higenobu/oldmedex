...
//0722-2012
//include_once $_SERVER['DOCUMENT_ROOT'].'/u/manage/index.php';
//0722-2012
//04-10-2012 changed for "病院使用医薬品名"
global $dp;
global $dirp;
global $auth;
global $pid;
...
//04-10-2012
  $stmt = <<<SQL
    ...
    O."startdate" + D."日数" >= '${new_order_start}' AND
...
function check_order_sanity($var,$check_ih=FALSE) {
  ...
  */
  ...
  // 05-12-2012 if all_generic_ok =0 ,at least one of them is 0
  if($var['igeneric'] == 1 and $all_generic_ok == 0)
...
//07-06-2012
   ...
   "window.open('print.php?oid={$oid}','','width=640,height=640')";
    ...
    print "<td><button type=\"button\" style=\"height:50\"  OnClick=\"$open_print_script\">ラベル印刷</button></td>\n";

//
  print "<td><button type=\"submit\" style=\"height:50\"  name='print2' value=\"$oid\">印刷2</button></td>\n";
    if($_mx_inhosp_rx_print) {
      $open_print2_script = 
	"window.open('print_inhosp.php?oid={$oid}','','width=640,height=640')";
      print "<td><button type=\"button\" style=\"height:50\"  OnClick=\"$open_print2_script\">院内印刷</button></td>\n";
    }
//0720-2012
//    if ($_mx_rx_control_print && $pt_outin == 'I')
      print "<td><button type=\"submit\" style=\"height:50\"  name='print_toukan'\" value=\"$oid\">投薬管理簿</button></td>\n";
    if ($_mx_rx_print)
      print "<td><button type=\"submit\" style=\"height:50\"  name='print' value=\"$oid\">印刷</button></td>\n";
    print "</tr></table>";
    print "<table>";
...
//04-2012 shiji
    $name = get_empid($ord['shiji']);
    if (!$_mx_hack_takamiya)
      $tr = get_teiki_rinji($ord['定期臨時']);
//0722-2012
    print "<td>{$name["姓"]}&nbsp;{$name["名"]}";
//

    print "<tr><th nowrap>処方日<td>{$ord['処方年月日']}
               <th nowrap>開始日<td>{$ord['startdate']}
           <td>{$ord['kubun']}${tr}";
//05-12-2012 if 0 then ka
    	if ($ord['generic']==0) print "<th>後発品可<td>" ;
	if ($ord['generic']==1) print "<th>後発品不可<td>" ;
...
//04-10-2012 "病院使用医薬品名"
    foreach (array('medis', 'レセプト電算処理システム医薬品名',
		   ...
		   '日数', '頓服','一包', '区分','定期臨時', 'accept', 'generic_ok', '用量単位','薬価基準収載医薬品コード','generic','startdate') as $key) {
  ...
  if ($_POST['dp-sel-id-select']) {
    ...
    $med = array();
    $med['medis'] = $k[0];
//-4-10-2012 "病院使用医薬品名"  this is a key place to dsplay kusuri at right plane
...
//05-12-2012
  ...
  $le->header = check_all_accepted($le->get_list(), $var['istartdate']);
...
//2012
	...
	// new order
	$ord = array();
	// set default value generic_ok
//05-12-2012 0-ka 1-fuka
	$ord["generic"] = 0;
	$ord['kubun'] = '院外';
	if ($_mx_rx_innai_ingai)
	  $ord['kubun'] = $_mx_rx_innai_ingai == 1 ? "院内" : "院外";
	else if($pt_outin == 'I') 
	  $ord['kubun'] = "院内";
	else if ($pt_outin == 'O')
	  $ord['kubun'] = "院外";
...
//new order -end
    ...
    if ($var["copy"]) { $ord["処方年月日"] = $ord["startdate"] = date("Y-m-d"); }
...
HTML;
    ...
    if ($_mx_rx_order_record_doctor) {
      if ($ord['ishiji'])
	    $rec['id'] = $ord['ishiji'];
      else if ($_REQUEST['ishiji'])
	    $rec['id'] = $_REQUEST['ishiji'];
      else
//0722-2012
	    $rec['id'] = $auth[2]['userid'];
    } else {
//0722-2012
      $rec['id'] = $auth[2]['userid'];
    }
...
//0722-2012
    $current_user['id'] = $auth[2]['userid'];
   $ename= get_empid($current_user['id']);
 $current_user['name'] =$ename["姓"];
$rec['name'] = $ename["姓"];
//
    if ($_mx_rx_use_set) {
      print ' セット登録';
...
# NEEDSWORK: add readonly mode by $readonly
    mx_formi_date("i処方年月日",
		  ($ord["処方年月日"] ? $ord["処方年月日"]
		   : date("Y-m-d")), NULL);
//0405-2012 add uesrid rather than ID
    if ($_mx_rx_order_record_doctor && $pid) {
      print '処方医';
      list_doctors2("ishiji",$rec['id'],NULL,"doctor",$current_user);
    } else {
      print "<input type=\"hidden\" name=\"ishiji\" value=\"{$rec['id']}\">";
    }

    if ($_mx_rx_use_set)
      print '<br />';
   print '開始日';
	mx_formi_date("istartdate",
		  ($ord["startdate"] ? $ord["startdate"]
		   : date("Y-m-d")), NULL);
    if($readonly != '') {
      print "\n";
     ...
     print '後発品 ';
//05-12-2012 if 0 then ka
    print '<input type=text value="'. ($ord['generic'] == 0 ? '可' : '不'). '" size=2 readonly>';
      print "\n";
      print '<input type=hidden name="igeneric" value="'.$ord['generic'].'">';
      print "\n";

	
//0730-2012 ikubun added
      print '区分 ';
      print '<input type=text name="ikubun" value="'.$ord['kubun'].'" size=4 readonly>';
      $tr = get_teiki_rinji($ord['定期臨時']);
...
// generic=0 -> generic is OK?
	if ($ord['generic']==0)  {
	print " generic OK";
	mx_formi_hidden("igeneric", $ord['generic']);
      }
      ...
      print '後発品(fuka:1)<select ONCHANGE="update_generic_ok(this)" name="igeneric"'.$__mx_formi_dek . '>
         <option value="1"' . ($ord['generic'] == 1 ? "selected" : "") . '>不可
         <option value="0"' . ($ord['generic'] == 0 ? "selected" : "") . '>可</select>';
 
/*
	print "後発品不可";
	mx_formi_checkbox("igeneric", $ord['generic'], NULL);
*/
      ...
      if ($fixed_kubun) {
	      print '区分: ' . $ord['kubun'];
	      mx_formi_hidden('ikubun', $ord['kubun']);
      } else {
	      print '区分 <select name="ikubun" '.$__mx_formi_dek. ' >';
...
//0730-2012
if ($pt_outin == 'O') {
		  print '<option value="外来" '.($ord['kubun'] == "外来" ? "selected" : "").'>外来';
		} else {
		  print '<option value="定期" '.($ord['kubun'] == "定期" ? "selected" : "").'>定期
		 <option value="臨時" '.($ord['kubun'] == "臨時" ? "selected" : "").'>臨時
		 <option value="退院時" '.($ord['kubun'] == "退院時" ? "selected" : "").'>退院時';
		}
		...
		print '<option value="院外" '.($ord['kubun'] == "院外" ? "selected" : "").'>院外
		 <option value="院内" '.($ord['kubun'] == "院内" ? "selected" : "").'>院内';
...
//0825-2012 vocab new line?
      if($_mx_meds_comment) {
	print '<br><b>処方箋コメント</b>';

        mx_formi_textarea("iComment", $ord['Comment'], array('rows'=>1,'cols'=>80));
      }
    }
	...
	$__mx_formi_dek.'>
      ...
      list_doctors2("i停止医",$ord['停止医'],$pid,"all0",$rec);
    }
    else {
...
//print '<body class="yui-skim-sam">';
//1116-2012
if($_REQUEST['print'])
  go_pdf($_REQUEST['print'], 0);
if($_REQUEST['print2'])
  go_pdf3($_REQUEST['print2'], 0);
//0720-2012
if($_REQUEST['print_toukan'])
  go_pdf3($_REQUEST['print_toukan'], 0, 'management_list_rx.ods');
