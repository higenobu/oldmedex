...
function check_order_sanity($var,$check_ih=FALSE) {
  ...
  */
  ...
  // 2012 on fuka
  if($var['igeneric'] == 'on' and $all_generic_ok == 1)
...
function show_rx ($var) {
  ...
  if ($oid) {
    ...
    print "<tr><th nowrap>処方日<td>{$ord['処方年月日']}
           ...
           <td>{$ord['区分']}${tr}";
    	if ($ord['generic']==1) print "<th>後発品可<td>" ;
	if ($ord['generic']==0) print "<th>後発品不可<td>" ;
...
# NEEDSWORK: add readonly mode by $readonly
   ...
   print '開始日';
    ...
    print '<input type=text value="'. ($ord['generic'] == 1 ? '可' : '不'). '" size=2 readonly>';
      print "\n";
