...
function get_drugs_in_past_orders($new_order_start, $tbl) {
  ...
  */
$tbl='xctorder';  
$table = array( array(xctorder),
		  array("注射処方箋", "注射処方箋内容") );
  
  $order_table = 'xctorder';
...
function show_rx ($var) {
  ...
  if ($oid) {
    ...
    print "</tr></table>";
    print "<table>";
    get_order_history(xctorder,$oid,"pill");
    print '</table>';
    print '<input type=hidden name="det-in" value="'.$oid.'">
           <table '.$class."><tr><th>orderid{$oid}";
    $name = get_emp_name($ord['記録者']);
    if (!$_mx_hack_takamiya)
      $tr = get_teiki_rinji($ord[teikikubun]);
    print "<td>{$name['lname']}&nbsp;{$name['fname']}";
    print "<tr><th nowrap>処方日<td>{$ord[orderdate]}
               <th nowrap>開始日<td>{$ord[plandate]}
           <td>{$ord[xctkubun]}${tr}";
    ...
    if($_mx_meds_comment){
      print '<tr><td><b>コメント</b></td><td colspan=3>'.$ord[memo1].'</td></tr>';
    }
