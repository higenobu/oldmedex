...
include_once $_SERVER['DOCUMENT_ROOT'].'/lib/u/pharmacy/shot_rx.php';
include_once $_SERVER['DOCUMENT_ROOT'].'/lib/u/pharmacy/shot_direction.php';
...
function check_all_accepted($le_list, $start_date=NULL) {
  ...
  if(is_array($ih_drugs) and count($ih_drugs) > 0) {
    ...
    foreach($ih_drugs as $ihd)
      $drug_codes[] = $ihd["レセプト電算処理システムコード（１）"];
    $ih = new Inhibit();
    $ih->check_combination($drug_codes);
...
function check_order_sanity($var,$check_ih=FALSE) {
  ...
  */
  ...
  // Rule 1
  $rows = $le->get_list();
  if (get_class($rows[0]) != 'shot') {
    return "先頭の行が薬剤ではありません";
  }
  // Rule 2
  if (get_class($rows[count($rows) - 1]) == 'shot' && 
      !$rows[count($rows) - 1]->external)
    return "手技を最後の行に追加してください";

  // Rule 3,4,5,6, pre7
  $all_generic_ok = 0;
  for($i=0; $i < count($rows); $i++) {
    if (get_class($rows[$i]) == 'shots_direction') {
      if (get_class($rows[$i-1]) != 'shot') {
	return sprintf("%d行目の手技(用法)は薬剤の後にしか入れられません", $i+1);
      }
      $dir = $rows[$i]->get_array();
      if (get_class($rows[$i-1]) == 'shot' && !$rows[$i-1]->external &&
	  (!$dir['頓服'] || $dir['頓服'] == 0) &&
	  (! $dir['日数'] || $dir['日数'] == '')) {
	return sprintf("%d行目の手技(用法)に、日数が入力されていません", $i+1);
      }
      if (get_class($rows[$i]) == 'shot') {
	$x = $rows[$i]->get_array();
...
function handle_db_action($var) {
  ...
  if ($dbaction == "dbnew") {
    ...
    if (!insert_pharm_order($var, &$new_oid_return, 1)) {
...
SQL;
  ...
  if ($dbok != '') {
	  ...
	  if ($dbaction == "dbstop") {
		  ...
		  $pt = $var['i患者'];
		  if ($_mx_orca_send_injection_on_orderdate)
			  $dt = $var['i処方年月日'];
		  else
			  $dt = $var['i処方開始日'];
	  }
...
function show_rx_history($pat,$var,$dp,$dirp, &$setlist) {
  ...
  print "<input type=\"hidden\" name=\"pid\" value=\"$pid\">\n
         ...
         新規処方作成</button>\n";
  if ($pid) {
    $drugpick_cfg = u_pharmacy_shots_order_drugpick_cfg($pt_outin);
    $drugpick_cfg['u'] = $u;
	...
	if($setlist->chosen_for_show) {
	  $meds = get_meds($setlist->chosen_for_show,1);
	  print '<pre>';
	...
	print "<br>\n";
	//手技選択
	mx_titlespan('手技選択', 'small_heading');
	$directionlist = new list_of_pharmacy_shot_methods('direction-list-');
	$directionlist->draw();
      }
    }
    ...
    if (!$show_edit_order || 1) {
      print "<br />\n";
      mx_titlespan('注射処方歴', 'small_heading');
      print "&nbsp;&nbsp;";
      $me = $_SERVER['PHP_SELF'];
      $match = array();
...
function show_rx ($var) {
  ...
  if ($oid) {
    ...
    $open_print_script = 
      "window.open('print.php?shots=1&oid={$oid}','','width=640,height=640')";
    print "<td><button type=\"button\" style=\"height:50\"  OnClick=\"$open_print_script\">注射ラベル印刷</button></td>\n";
    if($_mx_inhosp_rx_print) {
      $open_print2_script = 
	"window.open('print_inhosp.php?shots=1&oid={$oid}','','width=640,height=640')";
...
/* 10-20-2014
    if ($_mx_pdfgen_cmd != '')
      print "<td><button type=\"submit\" style=\"height:50\"  name='print' value=\"$oid\">注射箋印刷</button></td>\n";
*/


    print "</tr></table>";
    print "<table>";
    get_order_history("注射処方箋",$oid,"pill",1);
    print '</table>';
    ...
    print join("\n", set_body($meds,1,$ord['後発品']));
    print "\n";
    print_injection_exec_records($oid);
    print '</pre></table>
    </div>';
  }
}
  ...
  */
  for ($slot = 0; $slot < $var['max_row'] ; $slot++) {
    ...
    foreach (array('medis', 'レセプト電算処理システム医薬品名',
		   'レセプト電算処理システムコード（１）',
		   '用量', '用法分類','その他コメント', 'dosageid',
		   'methodid',
		   'unitid', '日数', '頓服','一包', '区分','定期臨時', 'accept', 'generic_ok', '用量単位') as $key) {
    ...
    if(array_key_exists('medis', $row)) {
      ...
      if($preview)
	$row['generic_ok'] = is_null($row['generic_ok']) ? 0 : 1;
      $le->add(new shot($row));
    }
    else
      $le->add(new shots_direction($row));
  }
  ...
  if ($_POST['dp-sel-id-select']) {
    ...
    $le->insert_at_cursor(new shot($med));
    ...
    $row = array();
    $row['methodid'] = $k[0];
    /* do we needs this for shots?
    if($k[1] == 1)
      $row['頓服'] = 0;
    */
    $le->insert_at_cursor(new shots_direction($row));
  }
...
function construct_meds(){
  ...
  ##################################################################
  $method_str = 'select "ObjectID", "手技" as val from "注射手技" where "Superseded" is NULL order by "ObjectID"';
  $methods = array();
  $res = pg_query($con,$method_str) or die(pg_last_error());
  while ($row = pg_fetch_assoc($res))
    $methods[$row['ObjectID']] = $row['val'];
  pg_free_result($res);
  ##################################################################
  $dosage_str = 'select "ObjectID", "用法" as val from "注射用法" where "Superseded" is NULL order by "ObjectID"';
  $dosages = array();
  $res = pg_query($con,$dosage_str) or die(pg_last_error());
  while ($row = pg_fetch_assoc($res))
    $dosages[$row['ObjectID']] = $row['val'];
  pg_free_result($res);
  ##################################################################
  ...
  foreach($le->get_list() as $row) {
    ...
    if($cls == 'shot'){
      ...
      $med['shapeid'] = 1;
      $meds[] = $med;
    }else if($cls == 'shots_direction'){
      foreach($row->get_array() as $k => $v){
	$meds[count($meds) -1 ][$k] = $v;
	if($k=='methodid')
	  $meds[count($meds) -1 ]['手技'] =
	    $methods[$v];
	if($k=='dosageid')
	  $meds[count($meds) -1 ]['注射用法'] =
	    $dosages[$v];
      }
    }
  }
  ...
  if ($var["new"]) {
    ...
    */
    ...
    if($setlist->chosen_for_do)
      $meds = get_meds($setlist->chosen_for_do,1);
    else
      $meds = get_meds($oid,1);
    if($meds) 
      foreach($meds as $row){
...
# NEEDSWORK: add readonly mode by $readonly
    ...
    if ($_mx_rx_order_record_doctor && $var['pid']) {
      print '処方医';
      list_doctors("i記録者",$rec['id'],$var['pid'],"doctor",$rec);
    } else {
...
function draw_main_content() {
  ...
  $setlist = new list_of_pharmacy_shots('set-list-', $pid, array('SETONLY' => 1));
  ...
  print "
   ...
   fitToBottom('div_past');
   fitToBottom('div_meds');
   </script>
   ";
}

function print_injection_exec_records($oid)
{
	global $_mx_show_exec_in_injection_order;

	if (!$_mx_show_exec_in_injection_order)
		return;

	$medsarray = get_meds($oid, 1);
	$opid = array();
	$meds = array();
	...
	$opid = implode(', ', $opid);
	$opid = 'AND "注射処方箋内容" IN (' . $opid . ')';
	$stmt = <<<SQL
		SELECT X."ObjectID", X."実施日", X."実施者",
		X."注射処方箋内容", X."特記事項", X.recorded,
		E."姓", E."名"
		FROM "注射実施記録" AS X
		LEFT JOIN "職員台帳" AS E
		ON E."ObjectID" = X."実施者"
...
$um = mx_get_usermode($u, 'rx_match_exp');
if(is_null($um))
  mx_set_usermode($u, 'rx_match_exp', ($_mx_rx_match_exp ? '1' : '2'));
...
mx_html_head($auth[1]);
print "<body onLoad=\"jumpTo('div_meds', 'ins_pos" . $le->cursor_pos;
print "');\">";
...
if ($pid) {
  $rxlist = new list_of_pharmacy_shots('rx-list-', $pid);
  if ($pid_changed)
    $rxlist->reset(NULL);
