...
$delstmt="delete from test_resultwk2";
...
/*	
$stmt = <<<SQL
insert into test_resultwk2 (karteno, testdate,ptname,komokucode,kekka,kijun1,kijun2,ijo) values ('$info[0]','$info[1]','$info[2]','$info[3]','$info[4]','$info[5]','$info[6]','$info[7]')
SQL;
...
$stmt = <<<SQL
update   test_resultwk2  set ptid=(select id from 
...
INSERT INTO test_order(
         "OrderDate", "SampleDate", 
          "ArrivalDate",  "Patient", printer )
    select max(date(testdate)), max(date(testdate)), date(now()), max(ptid) , '1'  from test_resultwk2 group by (testdate, ptid);
SQL;
...
select  m."ID",  r.kekka , r.ijo, r.ijo , r.ptid,  date(r.testdate) from test_master m, test_resultwk2 r 
...
r.ptid=t."Patient" and "SampleDate"=sampledate limit 1);
SQL;
